...
def plot_comparision2(
    df, generated, trajectories,
    palette = 'rainbow', df_time_key='samples',
    save=False, path=IMGS_DIR, file='comparision.png',
    x='d1', y='d2', z='d3', is_3d=False
):
    if not os.path.isdir(path):
...
def new_plot_comparisions2(
    ...
    new_cmap = LinearSegmentedColormap.from_list(
        'viridis_middle', cmap(np.linspace(start, end, 256))
    )
    cmap = new_cmap
    sns.set_palette(palette)
    plt.rcParams.update({
    ...
    for i, gs in enumerate(gspec):        
        ...
        for trajectory in np.transpose(trajectories, axes=(1,0,2)):
                plt.plot(trajectory[:, 0], trajectory[:, 1], alpha=1, color='red');
